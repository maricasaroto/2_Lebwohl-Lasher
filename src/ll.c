// criado em					: 2019/05/09
// ultima atualização	: 2019/07/19
// autor							: Mariana Casaroto <mariana.fcasaroto@gmail.com>
// notas							: Modelo de Lebwohl-Lasher
// compilação					: gcc ll.c -lm -lgsl -lgslcblas
...
const int     Nx =	  40;  // Tamanho da rede em x. 
const int     Ny =	  40;  // Tamanho da rede em y. 
const int     Nz =	  40;  // Tamanho da rede em z. 
...
int main(int argc, char **argv){
		 ...
		 int acc, ref;																							// Taxa de aceitação e de recusa.
	...
	double norma;																									// Norma do vetor aleatório
	...
	for(T= Ti; T<Tf; T+= DT){
 	  ...
 	  /* Transiente */
		for(s= 0; s<NTS; s++){
	      ...
	      n_r.nx= 1-2*gsl_rng_uniform(r);
  	    n_r.ny= 1-2*gsl_rng_uniform(r);
    	  n_r.nz= 1-2*gsl_rng_uniform(r);
	
  	    norma= sqrt((n_r.nx)*(n_r.nx)+(n_r.ny)*(n_r.ny)+(n_r.nz)*(n_r.nz));
